import type React from "react"
import type { Metadata } from "next"
import { Inter } from "next/font/google" //for Google fonts
import "./globals.css"
import { ThemeProvider } from "@/components/theme-provider"

const inter = Inter({ subsets: ["latin"] })

export const metadata: Metadata = {
  title: "GymBro - Fitness Logger",
  description: "Track your workouts, compete with friends, and improve your form",
  icons:"data:image/svg+xml;base64,",
} // Sets up the browser <title> and <meta description> for SEO and browser tabs

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode
}>) {
  return (
    <html lang="en">
      <body className={inter.className}>
        <ThemeProvider attribute="class" defaultTheme="dark" enableColorScheme> 
          {/* EnableColorScheme is like telling the browser: "Hey browser! I'm using dark/light mode classes (dark, light), so please automatically adjust scrollbars, form inputs, and other native UI elements** to match the theme I'm using." */}
          {children}
        </ThemeProvider>
      </body>
    </html>
  )
}
